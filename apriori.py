...
def scanD(D,Ck,minSupport):
  ...
  for tid in D:       #遍历数据集
    for can in Ck:    #遍历候选项
      if can.issubset(tid): #判断候选项中是否含有数据集的各项
        if not can in ssCnt:
          ssCnt[can]=1  #不含设为1
        else:
          ssCnt[can] +=1 #有则计数加1
...
def calcConf(freqSet,H,supportData,ruleList,minConf=0.7):
  for conseq in H: #遍历H中所有的项集并计算它们的可信度值
    conf = supportData[freqSet]/supportData[freqSet-conseq] # 可信度计算结合支持度数值
    #提升lift计算lift = p(a&b) /p(a)*p(b)
    lift = supportData[freqSet]/(supportData[conseq]*supportData[freqSet-conseq])
    if conf >=minConf and lift >1:
      print(freqSet-conseq,'--->',conseq,'支持度',round(supportData[freqSet],6),'置信度：',round(conf,6),'lift的值为：',round(lift,6))
      ruleList.append((freqSet-conseq,conseq,conf))

#生成规则
def gen_rule(L,supportData,minConf=0.7):
  bigRuleList = []
  for i in range(1,len(L)): #从2项集开始计算
    for freqSet in L[i]: #freqSet为所有的k项集
      #求该三项集的所有非空子集，1项集，2项集，到k-1项集，用H1表示，为list类型，里面为frozenset类型
      H1 = list(freqSet)
      all_subset = []
      getSubset(H1,all_subset) #生成所有子集
